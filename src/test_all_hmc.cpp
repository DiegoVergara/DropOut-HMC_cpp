...
int main(int argc, char const *argv[])
{
    ...
    if(argc != 21) {
        ...
        cout << "Lambda: " << lambda << ", Warmup: " << warmup << ", Iterations: " << iterations << ", Step Size: " << step_size << ", Num Steps:" << num_steps << ", Normalization: " << normalization <<", Standarization: " << standarization <<", Num Pred: " << num_pred <<endl;
		MatrixXd train, test;
        ...
        hmc.init(X_train, Y_train, lambda, warmup, iterations, minibatch,  step_size, num_steps, normalization, standarization, samples);
		...
		predicted_labels = hmc.predict(X_test, num_pred, false);

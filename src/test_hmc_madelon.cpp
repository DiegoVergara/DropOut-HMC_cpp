...
int main(int argc, char const *argv[])
{
	...
	MatrixXd train, test;
  	...
  	train_csv_path = "../data/MADELON/madelon.csv";
  	test_csv_path = "../data/MADELON/madelon_t.csv";
	...
	cout << "Lambda: " << lambda << ", Warmup: " << warmup << ", Iterations: " << iterations << ", Mini Batch: "<< mini_batch <<", Samples: " << samples << ", Step Size: " << step_size << ", Num Steps:" << num_steps << ", Mask: " << mask << endl;

	CPU_Hamiltonian_MC hmc;
	hmc.init(X_train, Y_train, lambda, warmup, iterations, mini_batch, step_size, num_steps, true, false, samples);
	...
	predicted_labels = hmc.predict(X_test, 30);

...
pred = genfromtxt(path+"/Y_hat.csv", delimiter=",")
...
max_gt_header = ["max_prob","Pred", "GT"]
header = np.concatenate((mean_header, max_gt_header, std_header), axis=0)
result = np.concatenate((prob_mean, np.reshape(prob_max,(-1,1)), np.reshape(pred,(-1,1)), np.reshape(gt,(-1,1)),prob_std),axis=1)
